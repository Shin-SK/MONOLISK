...
from .models import BillCastStay, Cast, Bill
...
def _req_store_id(request):
    sid = getattr(getattr(request, "store", None), "id", None) \
		  ...
		  or request.headers.get("X-Store-Id")  # ヘッダーの表記ゆれも許容
    try:
...
class CanOrderBillItem(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if hasattr(obj, 'table'):        # Bill
            ...
            bill = Bill.objects.select_related('table').only('id','table_id','table__store_id').get(id=obj.bill_id)
        else:
            return False

        # tableが未ロードなら補完
        if getattr(bill, 'table', None) is None:
            bill = Bill.objects.select_related('table').only('id','table_id','table__store_id').get(id=bill.id)
