...
import { fetchCastShiftHistory, fetchCastDailySummaries, createCastShift, updateCastShift, deleteCastShift, clearCastAttendance, fetchCastMypage } from '@/api'
...
async function ensureCastName () {
  // rows にあれば優先。なければ詳細APIから取得
  const n = rows.value[0]?.cast?.stage_name
  if (n && n !== stageName.value) {
    stageName.value = n
    return
  }
  try {
    const data = await fetchCastMypage(castId)        // billing/casts/{id}/
    if (data?.stage_name) stageName.value = data.stage_name
  } catch (e) {
    // 取得失敗時は空のまま（UIは壊れない）
    console.warn('fetchCastMypage failed', e)
  }
}
