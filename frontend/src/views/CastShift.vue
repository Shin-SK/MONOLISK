...
async function fetchAll () {
  /* ----- シフト行 ----- */
  list.value = await getShiftPlans({
    date : date.value,
    store: store.value || undefined        // '' の時は送らない
  }).then(rows =>
    rows.map(r => ({ ...r, date: r.date || date.value }))
  )

  /* ----- キャスト候補 ----- */
const params = {}
if (store.value) params.store = store.value   // 店舗指定がある時だけ絞る
casts.value = await api.get('cast-profiles/', { params }).then(r => r.data)
}
...
async function save (row) {
   ...
   // ② store を決定（プルダウン選択 or キャストから推定）
   let store_id = store.value
   if (!store_id && cast_profile) {
     const cp = casts.value.find(c => c.id === cast_profile)
     store_id = cp?.store           // ← API から返る store フィールド
   }

   const payload = { date, cast_profile, start_at, end_at, store: store_id }

   try {
     if (id) {
       await api.patch(`shift-plans/${id}/`, payload)
     } else {
       const { data } = await api.post('shift-plans/', payload)
       Object.assign(row, data)      // 返ってきた id / is_checked_in 等を反映
     }
     alert('保存しました！')
   } catch (e) {
     console.log(e.response?.data)   // 何かあればここに詳細が出る
