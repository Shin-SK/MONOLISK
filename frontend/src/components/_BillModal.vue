...
<template>
  <BaseModal v-model="visible">
    ...
    <div class="modal-body">
      <div class="wrapper d-grid" style="grid-template-columns: 1fr 1fr;">
        <div class="outer p-3">
          <div class="nomination mb-5">
            <div class="wrap d-flex gap-4"><!-- ★ボタンが押されたらprimaryになる感じに -->
              <input type="radio" class="btn-check" id="honshi">
              <label for="honshi" class="btn btn-outline-primary flex-fill">本指名あり</label>
              <input type="radio" class="btn-check" id="free">
              <label for="free" class="btn btn-outline-primary flex-fill">フリー</label>
            </div>
          </div>
          <!-- ───────── 選択結果 + 場内指定 ───────── -->
            ...
            <div class="d-flex p-3 bg-light justify-content-center gap-4">
              ...
              <template v-for="cid in nominatedCasts" :key="cid">
                <div class="btn btn-outline-primary">
                  ...
                  <span v-if="cid === mainCastId" class="badge bg-yellow ms-1">場内</span>
                  <input type="checkbox"
                    v-model="inhouseSet"
                    :value="cid"
          ...
          <div class="cast-area">
              <div class="d-flex tab-menu">
                <button class="nav-link active" data-bs-target="#select-honshi" type="button" role="tab">本指名</button> <!-- ★上の選択で、if honshiだったら、このボタンを表示 -->
                <button class="nav-link" data-bs-target="#select-free" type="button" role="tab">フリー</button>
              </div>
              
              <div class="tab-area"><!-- ★タブを表示させたい -->

                <!-- 本指名 -->
                <div class="area honshi-area fade show active" id="select-honshi"><!-- ★IDでつなげる -->
                  <h6 class="fw-bold">本指名</h6>
                ...
                <!-- フリー指名（チェックボックス複数） -->
                <div class="area honshi-area fade" id="select-free"><!-- ★IDでつなげる -->
                  <h6 class="mb-1">フリー</h6>
                  <div class="d-flex gap-4" role="group">
                    <!-- ★検索欄つけたい キャストの検索欄 名前だけでいいと思う -->
                    <template v-for="c in casts" :key="c.id">
                      <!-- hidden チェックボックス -->
          ...
          <div class="d-flex gap-2 align-items-end">
            <select class="form-select form-select-sm" v-model="draft.master_id">
