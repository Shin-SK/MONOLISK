<!-- src/components/cast/CastOrderModal.vue -->
<script setup>
...
onMounted(async () => {
  if (props.selectedBillId) {
    await pickBill(props.selectedBillId)
  } else if (props.candidates.length === 1) {
    await pickBill(props.candidates[0].id)
  }
  ready.value = true
})
...
<template>
  ...
  <!-- モーダル本体 -->
  <div class="modal fade show d-block" role="dialog" style="z-index: 1060;">
    <div class="modal-dialog modal-fullscreen p-2">
      <div class="modal-content d-flex flex-column">
        <div class="modal-header d-flex justify-content-between align-items-center flex-shrink-0">
        <div class="table-numb df-center gap-1">
            <span class="df-center badge bg-light text-dark small">
                <IconPinned />卓番号
            </span>
        ...
        <div class="modal-body p-2 position-relative flex-grow-1"
        style="overflow-y: auto;max-height:calc(100vh - 136px);">
          <div v-if="!ready" class="text-center py-5">読み込み中…</div>
        ...
        <div class="modal-footer border-top p-0">
          <div class="nav nav-pills nav-fill small gap-2 pills-flat w-100">
