...
<template>
  <header class="d-flex justify-content-between">
    ...
    <button
      class="btn btn-success align-items-center gap-1 mb-3"
      @click="newBill"
    >
      <IconPlus /> 新規追加
    </button>
    ...
    >
      ...
      >
        <div class="header rounded-top d-flex gap-3 bg-dark text-light py-2 px-4 align-items-center">
          <div class="item fs-4">
            <i class="bi bi-geo-fill" />{{ t.number }}
          </div>
          <div class="item ms-auto">
            <i class="bi bi-journal" />{{ getOpenBill(t.id).id }}
          </div>
          <div class="item">
            <i class="bi bi-person-fill" />{{ paxCount(getOpenBill(t.id)) ?? '‑' }}
          </div>
          <div class="item">
            {{ getOpenBill(t.id).set_rounds || '‑' }}SET
          </div>
        </div>
        ...
        <div class="content p-3 d-flex flex-column h-100">
          <div class="casts d-flex flex-wrap gap-2 mb-3">
            <div
              v-for="p in liveCasts(getOpenBill(t.id))"
              :key="p.id"
          ...
          <div class="time fs-2 mt-auto d-flex gap-2 align-items-center">
            <span class="fs-5 me-1"><i class="bi bi-clock" /></span>
            <span class="fw-bold">
      ...
      <div
        v-else
        class="table-box bg-light rounded d-flex flex-column
