...
const today     = computed(() => dayjs(useNow({interval:60_000}).value).format('YYYY.MM.DD (ddd)'))
...
<template>
  <div class="base owner d-block d-md-flex min-vh-100">
    <div class="sidebar d-flex gap-5 align-items-center">
      <button><RefreshAvatar /></button>

      <RouterLink class="nav-link" :to="{name:'owner-pl-daily'}"   :class="isActive('/dashboard') ? 'bg-dark text-white' : 'text-dark'"><span class="monst">D</span></RouterLink>
      <RouterLink class="nav-link" :to="{name:'owner-pl-monthly'}" :class="isActive('/dashboard/list') ? 'bg-dark text-white' : 'text-dark'"><span class="monst">M</span></RouterLink>
