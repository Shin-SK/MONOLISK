...
function handleAlertClick(bill) {
  closeAlertModal()
  router.push({ name: 'staff-bill-table', query: { billId: bill.id } })
}
  ...
  <div class="base managers d-block d-md-flex min-vh-100"
          ...
          >
            ...
            <div class="modal-body py-3">
              <div v-if="urgentBills.length > 0" class="alert-list">
                ...
                >
                  <router-link
                    :to="{ name: 'staff-bill-table', query: { billId: bill.id } }"
    ...
    style="z-index: 99;">
      <button 
        ...
        :class="isActiveName('staff-mypage') ? 'text-dark' : 'text-secondary'"
        @click="$router.push({  name: 'staff-mypage' })">
        <span>
          <IconHomeFilled v-if="isActiveName('staff-mypage')" />
          <IconHome v-else />
        </span>
        <small>ホーム</small>
      </button>
      <button 
        class="df-center flex-column"
        :class="isActiveName('staff-bill-table') ? 'text-dark' : 'text-secondary'"
        @click="$router.push({ name:'staff-bill-table' })">
        <span>
          <IconPinnedFilled v-if="isActiveName('staff-bill-table')" />
          <IconPinned v-else />
        </span>
        <small>卓伝票</small>
      </button>
      <button 
        class="df-center flex-column"
        :class="isActiveName('staff-bill-list') ? 'text-dark' : 'text-secondary'"
        @click="$router.push({ name:'staff-bill-list' })">
        <span>
          <IconLayoutListFilled v-if="isActiveName('staff-bill-list')" />
          <IconLayoutList v-else />
        </span>
        <small>伝票一覧</small>
      </button>
      <button class="df-center flex-column text-secondary" @click="openSidebar" aria-controls="staffSidebar">
        <span>
          <IconList />
        </span>
        <small>設定</small>
