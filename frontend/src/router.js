...
const routes = [
  ...
  // ---------- オーナー ---------- //
  {
    ...
    children: [
      ...
      { path: 'dashboard', name: 'owner-dashboard', component: () => import('@/views/owner/OwnerDashboard.vue'), meta:{ rolesAny: ['owner','superuser'], title:'マイページ' } },
      { path: 'bill/list', name:'owner-bill-list', redirect: { name: 'owner-dashboard', query: { tab: 'bills' } }, meta: { title: '伝票一覧', rolesAny: ['owner','superuser'] } },
      { path: 'bill', name:'owner-bill-table', component: () => import('@/views/DashboardAdmin.vue'), meta: { title: '卓伝票', rolesAny: ['manager','staff', 'owner' ,'superuser'], requiresAuth: true, adminOnly: true }},
